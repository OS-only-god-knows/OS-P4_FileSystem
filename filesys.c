...
void ScanBootSector()
{
	unsigned char buf[BOOT_SECTOR_SIZE];
	int ret, i;
	//将BootSector读入缓冲区
	if ((ret = read(fd, buf, BOOT_SECTOR_SIZE)) < 0)
	...
	FAT_SIZE = bdptor.BytesPerSector * bdptor.SectorsPerFAT;
	FAT_ONE_OFFSET = bdptor.BytesPerSector * bdptor.ReservedSectors;
	FAT_TWO_OFFSET = FAT_ONE_OFFSET + FAT_SIZE;
	ROOTDIR_OFFSET = FAT_TWO_OFFSET + FAT_SIZE;
...
int cheak()
{
	struct Entry *pentry, *start, *save;
	Dir *tmp;
	int ret, i;
	if (!strcmp(multdir->name, ".")) {
		start = curdir;
	}
	if (!strcmp(multdir->name, "/")){
		start = NULL;
	}
	for (tmp = multdir->next; tmp != NULL; tmp = tmp->next){
		pentry = (struct Entry*)malloc(sizeof(struct Entry));
		save = curdir;
		curdir = start;
		ret = ScanEntry(tmp->name, pentry, 1);
		curdir = save;
		if (ret < 0)
		{
			printf("no such dir\n");
...
int WriteFat()
{
	...
	for (i = 0, offset = FAT_ONE_OFFSET; i < bdptor.FATs; i++, offset += FAT_SIZE){
...
int ReadFat()
{
	...
	if (read(fd, fatbuf, CLUSTER_SIZE) < 0)
	{
		perror("read failed");
		return -1;
	}
...
int setDirEntry(int cluster)
{
	int i, dir_addr;
	unsigned char buf[CLUSTER_SIZE];
	dir_addr = DATA_OFFSET + (cluster - 2) * CLUSTER_SIZE;

	for (i = 0; i < CLUSTER_SIZE; i++)
		buf[i] = 0x00;

	if (lseek(fd, dir_addr, SEEK_SET) < 0) {
		perror("lseek failed");
		return -1;
	}
	if (write(fd, buf, CLUSTER_SIZE) < 0) {
		perror("write failed");
		return -1;
	}
	return 0;
...
int fd_mkdir(char *dirname)
{
	...
	if (ret < 0) {
		//在File Allocation Table中找到空⽩白簇，并标记为FFFF。
		for (cluster = 2; cluster < 1000; cluster++) {
			index = cluster * 2;
			if (fatbuf[index] == 0x00 && fatbuf[index + 1] == 0x00) {
				fatbuf[index] = 0xff;
		...
		if (curdir == NULL) {
			...
			while (offset < DATA_OFFSET) {
				...
				/*找出空目录项或已删除的目录项*/
					...
					/*写文件的大小*/
					c[28] = (32 & 0x000000ff);
					c[29] = ((32 & 0x0000ff00) >> 8);
					c[30] = ((32 & 0x00ff0000) >> 16);
					c[31] = ((32 & 0xff000000) >> 24);
...
int isEmpty(struct Entry *dir)
{
	int ret, i, dir_addr, offset;
	struct Entry entry;

	dir_addr = (dir->FirstCluster - 2) * CLUSTER_SIZE + DATA_OFFSET;
	if ((ret = lseek(fd, dir_addr, SEEK_SET)) < 0) {
		perror("lseek dir_addr failed");
		return -1;
	}

	offset = dir_addr;

	while (offset < dir_addr + CLUSTER_SIZE) {
		ret = GetEntry(&entry);
		offset += abs(ret);
		if (ret > 0)
			return 1;
...
char *subStr(int i, int j, char *str)
{
	int t;
...
int findCh(int start, char ch, char *str)
{
	int i;
...
void sbtime()
{
	...
	p = localtime(&timep);
	tmp = p->tm_min;
	fuck[1] = (unsigned char)p->tm_hour << 3 | tmp >> 3;
	fuck[0] = (unsigned char)p->tm_sec / 2 | (unsigned char)p->tm_min << 5;
	tmp = p->tm_mon+1;
	fuck[3] = (unsigned char)(p->tm_year - 2000) << 1 | tmp >> 3;
	tmp = p->tm_mon+1;
	fuck[2] = tmp << 5 | (unsigned char)p->tm_mday;
}
